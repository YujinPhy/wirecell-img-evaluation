...
`scripts/utils/true_blob.py`가 헤더 주석에서 `wirecell.util.wires.persist`/`schema`를 참조한다고 밝힌 부분을 근거로, 그 두 모듈의 실제 동작과 wire geometry(와이어 기하 정보)의 데이터 모델, 그리고 `utils/wires.py`가 이를 소비하는 방식을 정리한 문서다.
`wire-cell-python/wirecell/util/wires/schema.py`, `persist.py`가 소스 위치다.

관련 로직을 파이썬으로 포팅하여 활용가능하게 구현한 코드는 `scripts/utils/wires.py`이며 해당 코드에 대한 설명은 `docs/wires_geometry_walkthrough.md`을 참고하면 된다.
...
### 2.3 `ident`의 의미
대부분의 객체가 `ident` 필드를 갖는데, 이는 C++ 쪽 코드에도 그대로 노출되지만 툴킷 입장에서는 불투명하지만 유일한 값으로만 취급된다.
